...
def is_nodule_like_mask(mask, min_area=5, max_area=400, min_circularity=0.20, min_solidity=0.70, max_elongation=2.0,):
...
def analyze_CT_volume_sam2_slice_wise_all_modes(
    ...
    return (dsc_auto_vol, dsc_point_vol, dsc_box_vol, dsc_combined_vol, all_scores_points, all_scores_boxes, all_scores_combined, pred_auto_vol, pred_point_vol, pred_box_vol, pred_combined_vol, gt_volume,)


def analyze_CT_volume_sam2_slice_wise_3ch(
    mhd_file,
    path_volumes,
    path_masks,
    df_ids_link,
...
:
    ...
    for slice_info in slice_prompt_dicts:
        ...
        img_3ch = build_sam2_input_slice(
            image_array=image_array,
            z=z,
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            # slice accumulators per channel
...
def main():
    ...
    for mhd_file in tqdm.tqdm(only_name_files):
        ...
        result = analyze_CT_volume_sam2_slice_wise_3ch(
            mhd_file=mhd_file,
            path_volumes=path_volumes,
            path_masks=path_masks,
            df_ids_link=df_ids_link,
            predictor=predictor,   # SAM2ImagePredictor
    ...
    path_save_results = os.path.join(path_dataset, "predictions_sam2_img_multimask_results_LUNA16.csv")
